    ...
    def __call__(self, predictor, prompts, data, agg='mean', max_threads=1):
        def compute_scores(prompts_exs):
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=max_threads) as executor:
                futures = [executor.submit(self.predict_on_example, ex) for ex in inputs]
...
class Cached01BatchScorer:
    ...
    def __call__(self, predictor, prompts, data, agg='mean', max_threads=1):
        def compute_scores(prompts_exs):
            ...
            #         else:
            #             out_scores[f'{ex}-{prompt}'] = 0
            prompts, exs, preds = self.predict_on_example(predictor,prompts_exs)
            # print(preds)
            # print([ex['label'] for ex in exs])
            for prompt, ex, pred in zip(prompts, exs, preds):
                if pred == ex['label']:
                    out_scores[f'{ex}-{prompt}'] = 1
                else:
                    out_scores[f'{ex}-{prompt}'] = 0
    ...
    def predict_on_example(self, predictor, inputs):
        prompts, exs = [i[0] for i in inputs], [i[1] for i in inputs]
        texts = [e['text'] for e in exs]
        print("len of prompts, texts")
        print(len(prompts))
        print(len(texts))
        preds = predictor.inference(texts, prompts)
        return prompts, exs, preds